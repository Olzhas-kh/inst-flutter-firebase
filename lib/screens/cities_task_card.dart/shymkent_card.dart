...
class ShymkentCard extends StatefulWidget {
  final snap;
  const ShymkentCard({Key? key, required this.snap}) : super(key: key);

  @override
  State<ShymkentCard> createState() => _ShymkentCardState();
}

class _ShymkentCardState extends State<ShymkentCard> {
  bool isbutton = true;
  final listStatus = [
    '',
    'В процессе: ',
  ...
  getData() async {
    ...
    try {
      ...
      var userSnap =
          await FirebaseFirestore.instance.collection('shymkent').doc(widget.snap['commentId']).get();
         ...
         widget.snap['likes'].toString()=="[]"?
          MaterialButton(
              onPressed:  (){
                  ...
                  FireStoreMethods().statusTaskShymkent(widget.snap['commentId'], 'В процессе:',widget.snap['status']);
            ...
            children: <Widget>[
              ...
              onPressed:  (){
                if(widget.snap['likes'].contains(user.username)){
                 ...
                 FireStoreMethods().statusTaskShymkent(widget.snap['commentId'], 'Выполнено:',widget.snap['status']);
                 FireStoreMethods().statusTaskShymkent(widget.snap['commentId'], 'В процессе:',widget.snap['status']);
                  }else
                  {
                    showSnackBar('В процессе', context);
